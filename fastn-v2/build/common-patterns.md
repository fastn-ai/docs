...
**Ask for it as:** _"Make this idempotent — if the same order comes in twice, only create it once."_
...
**Ask for it as:** _"Only write records whose fields actually changed since the last run."_
...
**Ask for it as:** _"Pull into the database first, then push in batches of 200."_
...
Configs vary by _environment_, not by customer — so a per-customer rule belongs in the database or in a customer-scoped secret, not in a config.
...
**Ask for it as:** _"Make the warehouse and the minimum order value configurable per customer, not hard-coded."_
...
**Ask for it as:** _"Fan this webhook out to three workflows."_
...
**Ask for it as:** _"A one-off backfill for the last two years, safe to run alongside the hourly sync."_

***

## Workspace-level notifications

**The problem.** You want failures posted to _your_ Slack, not the customer's.

**The shape.** Use a Slack connection your organisation owns — one created from **New connection** and scoped `Account level` — rather than one belonging to a customer. On the workflow's [Connectors tab](workflows.md) it is the row _without_ a **Per customer** badge, so it behaves the same no matter whose data is being processed.
...
**Ask for it as:** _"Retry this three times with backoff if the destination is down."_
...
**Ask for it as:** _"Create the contact through the unified CRM endpoint, not through HubSpot directly."_
...
**Ask for it as:** _"A nightly check that both systems have the same number of open orders, and tell me when they don't."_
