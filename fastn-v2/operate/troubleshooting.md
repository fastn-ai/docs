...
| It worked yesterday and not today        | [Connections](../build/connections.md), then [Pending updates](../build/pending-updates.md) |
...
2. **The upstream API changed.** Check [Pending updates](../build/pending-updates.md) for a proposal against that connector.
...
Open [Sync reports](sync-reports.md). It shows what the run did, record by record — which distinguishes _the record was filtered out_ from _the record was never seen_.
...
See [Workflow runtime API](../reference/workflow-runtime-api.md).
...
Use the search box rather than the status filter chips: see [Known issues](troubleshooting.md#known-issues) below.
