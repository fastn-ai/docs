...
![A workflow's API tab](https://raw.githubusercontent.com/fastn-ai/docs/docs-v2/.gitbook/assets/workflow-api-tab.jpg)

_Every workflow's API tab shows its own endpoint and a copyable curl._
...
Inside the workflow these arrive on `ctx.headers` — see [Workflow runtime API](workflow-runtime-api.md). Exact value formats are on each workflow's own **Docs** tab, generated from the deployment you are calling; check there before you hard-code one.
...
That URL carries a live credential. Treat it like one — build it server-side per session, and do not log or share it. Full setup in [Embedding the widget](../embed/embedding-the-widget.md).
...
Per-day and per-minute ceilings on API calls and events are listed, with current usage, on [Settings → Billing](../manage/billing-and-limits.md), which also carries per-customer limits. Going over stops new work rather than charging you, and nothing already running is interrupted — so a caller that suddenly gets nowhere is worth checking against that page before you debug the workflow.
