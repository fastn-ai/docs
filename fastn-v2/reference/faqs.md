...
No. Your customers work through the [widget](../embed/widgets.md) embedded in your product, authorising their own accounts there. They reach it with an embed token your backend mints, which carries the role `end_user` — not a fastn login of their own.
...
They are the same thing under two names, and **both are current**. _Customer_ is what the dashboard calls it — the Customers screen, the ⌘K search group, the Connections column. _Tenant_ is what the plumbing calls it: it is the column header on all three Triggers tables, and the last segment of a connection id, `ucl:org_<org>:<env>:<connectorId>:<authId>:<tenant>`.

So do not read _tenant_ as legacy vocabulary you can ignore. If a screen or an identifier says tenant, it means one of your customers.
...
The precise meaning of each status is not documented here; treat _Expired_ and _Failed_ as "this credential no longer works, ask the customer to reconnect" and read the connection's own **Token and activity** section for `Expires`, `Last refreshed` and `Last used`.
...
`test` runs the workflow's latest published version. Any named environment runs the version deployed _there_, and the fire fails if nothing is deployed to it.
...
Set a **deduplication key** on the webhook trigger, and add an idempotency guard using `fastn.state` in the workflow. See [Common patterns](../build/common-patterns.md).
...
Check whether the workflow has ever been published. Until a snapshot exists, every call returns `WORKFLOW_NOT_PUBLISHED` — the workflow list shows this as status **Not published** and latest version **Unpublished**. See [HTTP API](http-api.md).
...
No. Browser-facing widgets use short-lived embed tokens minted by your backend. See [Embedding the widget](../embed/embedding-the-widget.md).
...
New work stops rather than being charged for, and nothing already running is interrupted. Check [Billing](../manage/billing-and-limits.md) — a sync that stops because of a quota looks exactly like a broken sync until you do.
...
The practical consequence: the connector count is a count of _entries_, not of distinct systems, so "236 connectors" is not 236 different products. Check the badge and the provenance line before you connect, so you do not authorise the copy you did not mean.
...
Known: the **Actions** tab on [Trash](../manage/trash.md) sits on _Loading deleted actions…_ and does not resolve. The Connectors and Workflows tabs work normally.
