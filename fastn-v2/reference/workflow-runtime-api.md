...
`ctx.headers` is also how a run knows which of your customers it is acting for — see [Multi-tenant headers](workflow-runtime-api.md#multi-tenant-headers) below.
...
The slug is the one on the connector's Overview tab. Available actions and their versions are pinned on the workflow's **Connectors** tab, and each connector there is marked _per customer_ or workspace — which decides whose credential the call uses.
